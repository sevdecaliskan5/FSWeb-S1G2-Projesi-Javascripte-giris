...
function bilgisayarinSecimi() {
  ...
  const rastgeleSayi = Math.ceil(Math.random() * 3);
...
function milDonusturucu(sayi) {
  const mil = 0.621371*sayi;
  return mil;
}
milDonusturucu(sayi);
...
function feetDonusturucu(sayi) {
  const feet = 0.0328084*sayi;
  return feet;
}
feetDonusturucu(sayi);
